...
These data define a structure called MChit, which is later used by the reconstruction algorithm to reproduces hit in the same format of the data. The algorithm for the reconstruction works as follow: first, only hits with a minimal energy deposit in the volume are considered, since a minimum amount of energy need to be deposited to start the ionization inside the volumes. After that, multiple MChit are merged if they are found to be closer than a certain distance. This distance is conservatively set to be 2 mm (see Sec.\ref{ch5:sec:separ-hit-micr}). Hit closer than distance are merged in a single one and the new x-y position are computed by weighting each position by its energy deposit in the gas.
