...
Finally, we will provide a description of both the analysis performed for the invisible and visible mode setup in Sec.\ref{ch3:sec:analysis-invis} and Sec.\ref{ch3:sec:analysis-vis} respectively. In the case of the visible mode, this section also includes a new analysis which uses the information recorded by the trackers on top of the one of the calorimeters.
For both analyses, the list of the selection criteria is provided, then the background is discussed in detailed. We show that in both modes the background is expected to be under control, with an average of expected event in the signal region of $\simeq 0.5$ for the invisible mode and $\simeq 0.01$ for the visible mode.
...
These data define a structure called \textit{MChit}, which is used by the reconstruction algorithm to reproduce both hits and tracks in the same format of the data. The algorithm for the reconstruction works as follow:
\begin{enumerate}

\item Hits with an energy deposit in the volume smaller than a threshold are removed, since a minimum amount of energy needs to be deposited to start the ionization inside the gas. Practically, the minimum threshold was set to 2 keV based on previous studies performed for MM \cite{IGUAZ20121079}. A similar value is assumed for the GEM detector as well.
...
In this section, we give a brief overview of the two main methods used to reject hadrons (and muons) contamination in the beam developed as a consequence of my work.
